...
/* keeps track of where the neighbor is */
int *neighbor;}gather_t;
...
static int n_gathers;
static int gather_array_len;
/**********************************************************************
...
void sort_eight_gathers(int index)
{
  ...
  for (i = 0; i <= 3; i++) {
    ...
    memcpy((gather_array + (index + (7 - i))),(tt + ((2 * i) + 1)),(sizeof(gather_t )));
  }
}
/*
...
int *dirpt,
/* "forwards/backwards"  */
int fb,int *x2p,int *y2p,int *z2p,int *t2p)
...
void make_nn_gathers()
{
  ...
  if (gather_array == ((gather_t *)((void *)0))) {
    printf("error: not enought room for gather_array in make_nn_gathers\n");
    terminate(1);
  }
  if (((((nx & 1) != 0) || ((ny & 1) != 0)) || ((nz & 1) != 0)) || ((nt & 1) != 0)) 
    gather_parity = 2;
  else 
    gather_parity = 1;
  for (i = 0; i <= 3; i++) 
    make_gather(neighbor_coords_special,&i,1,0,gather_parity);
...
int *args,
/* OWN_INVERSE, WANT_INVERSE, or NO_INVERSE */
int inverse,
/* ALLOW_EVEN_ODD or NO_EVEN_ODD */
int want_even_odd,
/* {SAME,SWITCH,SCRAMBLE}_PARITY */
int parity_conserve)
{
/* scratch */
  int i;
...
/* coordinates */
  ...
  int z;
  int t;
/* sublist of sender for a given receiver */
  int *send_subl;
...
/* lengthen gather array to add more gathers */
    gather_array = ((gather_t *)(realloc(gather_array,(gather_array_len * sizeof(gather_t )))));
  }
/* index of gather we are working on */
  dir = (n_gathers - 1);
  gather_array[dir].neighbor = ((int *)(malloc((sites_on_node * sizeof(int )))));
  if (gather_array[dir].neighbor == ((int *)((void *)0))) {
...
/* index of gather we are working on */
  ...
  if ((want_even_odd == 0) && (parity_conserve != 2)) {
    send_subl = ((int *)(malloc((2 * sizeof(int )))));
    if (send_subl == ((int *)((void *)0))) {
...
/* find coordinates of neighbor who sends us data */
    ( *func)((s -> x),(s -> y),(s -> z),(s -> t),args,1,&x,&y,&z,&t);
    ...
    if (parity_conserve != 2) {
      ...
      if ((parity_conserve == 0) && (s_subl != r_subl)) {
      ...
      if ((parity_conserve == 1) && (s_subl == r_subl)) {
        printf("DUMMY! Your gather mapping does not obey claimed parity");
...
/* inverse gather has direction one more than original */
  dir++;
...
static msg_tag *declare_strided_gather(
/* source buffer aligned to desired field */
void *field,
/* bytes between fields in source buffer */
int stride,
/* size in bytes of the field (eg sizeof(su3_vector))*/
int size,
/* direction to gather from. eg XUP - index into
			   neighbor tables */
int index,
/* subl of sites whose neighbors we gather.
			   It is EVENANDODD, if all sublattices are done. */
int subl,
/* one of the vectors of pointers */
char **dest)
{
/* scratch */
  int i;
/* scratch pointer to site */
  site *s;
/* pointer to current gather */
  gather_t *gt;
...
/* set pointers in sites whose neighbors are on this node.  (If all
  ...
  if (subl == 3) {
    ...
    for (((i = ((subl == 1)?even_sites_on_node : 0)) , (s = (lattice + i))); i < (((subl == 2)?even_sites_on_node : sites_on_node)); (i++ , s++)) {
      if ((gt -> neighbor)[i] != -1) {
        dest[i] = (((char *)field) + ((gt -> neighbor)[i] * stride));
      }
    }
...
void do_gather(
/* previously returned by start_gather */
msg_tag *mtag)
{
}
/*
...
/* which field? Some member of structure "site" */
field_offset field,
/* size in bytes of the field (eg sizeof(su3_vector))*/
int size,
/* direction to gather from. eg XUP - index into
			   neighbor tables */
int index,
/* parity of sites whose neighbors we gather.
			   one of EVEN, ODD or EVENANDODD. */
int parity,
/* one of the vectors of pointers */
char **dest)
{
  return declare_strided_gather((((char *)lattice) + field),(sizeof(site )),size,index,parity,dest);
}
/*
...
{
  msg_tag *mt;
  mt = declare_strided_gather((((char *)lattice) + field),(sizeof(site )),size,index,parity,dest);
  prepare_gather(mt);
  do_gather(mt);
  return mt;
}
...
/* which field? Pointer returned by malloc() */
void *field,
/* size in bytes of the field (eg sizeof(su3_vector))*/
int size,
/* direction to gather from. eg XUP - index into
...
void declare_accumulate_gather(msg_tag **mmtag,
/* which field? Some member of structure "site" */
field_offset field,
/* size in bytes of the field (eg sizeof(su3_vector))*/
int size,
...
char **dest)
{
  declare_accumulate_strided_gather(mmtag,(((char *)lattice) + field),(sizeof(site )),size,index,parity,dest);
}
/*
**  declares and merges gather from temp
*/

void declare_accumulate_gather_from_temp(msg_tag **mmtag,
/* which field? Pointer returned by malloc() */
void *field,
/* size in bytes of the field (eg sizeof(su3_vector))*/
int size,
...
char **dest)
{
  declare_accumulate_strided_gather(mmtag,((char *)field),size,size,index,parity,dest);
}
/**********************************************************************
   ...
   example:
	...
	cleanup_general_gather(tag);
	  ** subsequent calls will overwrite the gathered fields. but if you
	   don't clean up, you will eventually run out of space **
*/
/* flag to tell if general gather in progress */
static int g_gather_flag = 0;
...
/* size in bytes of the field (eg sizeof(su3_vector))*/
int size,
/* displacement to gather from. four components */
int *displacement,
/* subl of sites whose neighbors we gather.
			   It is EVENANDODD, if all sublattices are done. */
int subl,
...
/* set pointers in sites whose neighbors are on this node.  (If all
  ...
  if (subl == 3) {
    for (((i = 0) , (s = lattice)); i < sites_on_node; (i++ , s++)) {
      if (displacement[0] != 0) 
        tx = ((((s -> x) + displacement[0]) + nx) % nx);
      else 
        tx = (s -> x);
      if (displacement[1] != 0) 
        ty = ((((s -> y) + displacement[1]) + ny) % ny);
      else 
        ty = (s -> y);
      if (displacement[2] != 0) 
        tz = ((((s -> z) + displacement[2]) + nz) % nz);
      else 
        tz = (s -> z);
      if (displacement[3] != 0) 
        tt = ((((s -> t) + displacement[3]) + nt) % nt);
      else 
        tt = (s -> t);
...
/* displacement to gather from. four components */
int *displacement,
/* parity of sites to which we gather.
			   one of EVEN, ODD or EVENANDODD. */
int parity,
/* one of the vectors of pointers */
char **dest)
{
  return start_general_strided_gather((((char *)lattice) + field),(sizeof(site )),size,displacement,parity,dest);
}
...
char **dest)
{
  return start_general_strided_gather(((char *)field),size,size,displacement,parity,dest);
}
/*
