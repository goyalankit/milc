...
/* Scratch space */
/* malloced diffmat pointer */
su3_matrix *diffmatp;
/* malloced sumvec pointer */
su3_vector *sumvecp;
/* field offsets */
...
void do_hit(int gauge_dir,int parity,int p,int q,float relax_boost,int nvector,field_offset vector_offset[],int vector_parity[],int nantiherm,field_offset antiherm_offset[],int antiherm_parity[])
{
...
/* vector <- u * vector */
      if ((vector_parity[j] == 3) || (vector_parity[j] == parity)) 
        mult_su2_mat_vec_elem_n(&u,(( *((su3_vector *)(((char *)s) + vector_offset[j]))).c + p),(( *((su3_vector *)(((char *)s) + vector_offset[j]))).c + q));
/* Transform antihermitian matrices if requested */
    for (j = 0; j < nantiherm; j++) 
/* antiherm <- u * antiherm * u^dagger */
      if ((antiherm_parity[j] == 3) || (antiherm_parity[j] == parity)) {
        uncompress_anti_hermitian(((anti_hermitmat *)(((char *)s) + antiherm_offset[j])),&htemp);
...
void gaugefixstep(int gauge_dir,double *av_gauge_fix_action,float relax_boost,int nvector,field_offset vector_offset[],int vector_parity[],int nantiherm,field_offset antiherm_offset[],int antiherm_parity[])
{
...
/* Start gathers of downward links */
    for (dir = 0; dir <= 3; dir++) {
      mtag[dir] = start_gather(((field_offset )(((char *)(lattice[0].link + dir)) - ((char *)(lattice + 0)))),(sizeof(su3_matrix )),(7 - dir),parity,gen_pt[dir]);
    }
/* Wait for gathers */
    for (dir = 0; dir <= 3; dir++) {
...
/* Do optimum gauge hit on various subspaces */
    do_hit(gauge_dir,parity,0,1,relax_boost,nvector,vector_offset,vector_parity,nantiherm,antiherm_offset,antiherm_parity);
    do_hit(gauge_dir,parity,1,2,relax_boost,nvector,vector_offset,vector_parity,nantiherm,antiherm_offset,antiherm_parity);
    do_hit(gauge_dir,parity,2,0,relax_boost,nvector,vector_offset,vector_parity,nantiherm,antiherm_offset,antiherm_parity);
...
/* Gather diffmat onto sites of opposite parity */
      if (diffmat_offset >= 0) 
        mtag[dir] = start_gather(diffmat_offset,(sizeof(su3_matrix )),dir,(3 ^ parity),gen_pt[dir]);
      else 
        mtag[dir] = start_gather_from_temp(diffmatp,(sizeof(su3_matrix )),dir,(3 ^ parity),gen_pt[dir]);
      wait_gather(mtag[dir]);
/* Copy modified matrices into proper location */
      for (((i = (((3 ^ parity) == 1)?even_sites_on_node : 0)) , (s = (lattice + i))); i < ((((3 ^ parity) == 2)?even_sites_on_node : sites_on_node)); (i++ , s++)) 
...
void gaugefix(int gauge_dir,float relax_boost,int max_gauge_iter,float gauge_fix_tol,field_offset diffmat,field_offset sumvec,int nvector,field_offset vector_offset[],int vector_parity[],int nantiherm,field_offset antiherm_offset[],int antiherm_parity[])
{
  int gauge_iter;
  double current_av;
  double old_av;
...
/* the change in the avg gauge fixing action is smaller than gauge_fix_tol */
    for (gauge_iter = 0; gauge_iter < max_gauge_iter; gauge_iter++) {
      gaugefixstep(gauge_dir,&current_av,relax_boost,nvector,vector_offset,vector_parity,nantiherm,antiherm_offset,antiherm_parity);
      if (gauge_iter != 0) {
