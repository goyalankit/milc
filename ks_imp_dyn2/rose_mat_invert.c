...
int mat_invert_cg(field_offset src,field_offset dest,field_offset temp,float mass)
{
  int cgn;
  float finalrsq;
  clear_latvec(dest,3);
  cgn = ks_congrad(src,dest,mass,niter,rsqprop,3,&finalrsq);
/* Multiply by Madjoint */
  dslash_fn(dest,((field_offset )(((char *)(&lattice[0].ttt)) - ((char *)(lattice + 0)))),3);
  scalar_mult_add_latvec(((field_offset )(((char *)(&lattice[0].ttt)) - ((char *)(lattice + 0)))),dest,(-2.0 * mass),((field_offset )(((char *)(&lattice[0].ttt)) - ((char *)(lattice + 0)))),3);
  scalar_mult_latvec(((field_offset )(((char *)(&lattice[0].ttt)) - ((char *)(lattice + 0)))),(-1.0),dest,3);
  return cgn;
}
...
int mat_invert_uml(field_offset src,field_offset dest,field_offset temp,float mass)
{
  int cgn;
  float finalrsq;
  register int i;
...
/* multiply by U - even sites only */
  dslash_fn(src,((field_offset )(((char *)(&lattice[0].ttt)) - ((char *)(lattice + 0)))),2);
  scalar_mult_add_latvec(((field_offset )(((char *)(&lattice[0].ttt)) - ((char *)(lattice + 0)))),src,(-2.0 * mass),temp,2);
  scalar_mult_latvec(temp,(-1.0),temp,2);
/* invert with M_adj M even */
  cgn = ks_congrad(temp,dest,mass,niter,rsqprop,2,&finalrsq);
/* multiply by (1/2m)L, does nothing to even sites */
/* fix up odd sites , 1/2m (Dslash_oe*dest_e + phi_odd) */
  dslash_fn(dest,((field_offset )(((char *)(&lattice[0].ttt)) - ((char *)(lattice + 0)))),1);
  for (((i = even_sites_on_node) , (s = (lattice + i))); i < sites_on_node; (i++ , s++)) {
    ...
    scalar_mult_su3_vector(((su3_vector *)(((char *)s) + dest)),(1.0 / (2.0 * mass)),((su3_vector *)(((char *)s) + dest)));
  }
  return cgn;
}
/* FOR TESTING: multiply src by matrix and check against dest */

void check_invert(field_offset src,field_offset dest,float mass,float tol)
{
  register int i;
  register int k;
  register int flag;
  ...
  double sum;
  double sum2;
  dslash_fn(src,((field_offset )(((char *)(&lattice[0].cg_p)) - ((char *)(lattice + 0)))),3);
  for (((i = 0) , (s = lattice)); i < sites_on_node; (i++ , s++)) {
    scalar_mult_add_su3_vector(&s -> cg_p,((su3_vector *)(((char *)s) + src)),(+2.0 * mass),&s -> cg_p);
  }
  sum2 = (sum = 0.0);
  for (((i = 0) , (s = lattice)); i < sites_on_node; (i++ , s++)) {
    for (((flag = 0) , (k = 0)); k < 3; k++) {
      ...
      if ((fabs(r_diff) > tol) || (fabs(i_diff) > tol)) 
        flag = 1;
      if (flag != 0) 
        printf("%d %d  ( %.4e , %.4e )  ( %.4e , %.4e )\n",i,k,( *((su3_vector *)(((char *)s) + dest))).c[k].real,( *((su3_vector *)(((char *)s) + dest))).c[k].imag,s -> cg_p.c[k].real,s -> cg_p.c[k].imag);
      if (flag != 0) 
        terminate(0);
      sum += ((r_diff * r_diff) + (i_diff * i_diff));
    }
    sum2 += (magsq_su3vec(((su3_vector *)(((char *)s) + dest))));
  }
  g_doublesum(&sum);
  g_doublesum(&sum2);
  g_sync();
