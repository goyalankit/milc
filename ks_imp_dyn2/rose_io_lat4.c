...
int big_endian()
{
  union __unnamed_class___F0_L143_C3_L70R_variable_declaration__variable_type_l_variable_name_L70R__scope__l__DELIMITER__L70R_variable_declaration__variable_type__Ab_c_index_8_Ae__variable_name_L70R__scope__c {
  long l;
  char c[8UL];}u;
  u.l = 1;
...
/* Allocate space for QCDheader and its pointers */
  tokens = ((char **)(malloc((512 * sizeof(char *)))));
  values = ((char **)(malloc((512 * sizeof(char *)))));
...
void swrite_data(FILE *fp,void *src,size_t size,char *myname,char *descrip)
{
  if (fwrite(src,size,1,fp) != 1) {
    printf("%s: Node %d %s write error %d\n",myname,this_node,descrip, *__errno_location());
    fflush(stdout);
    terminate(1);
  }
}
...
void pswrite_data(int parallel,FILE *fp,void *src,size_t size,char *myname,char *descrip)
{
  if (parallel != 0) 
...
int psread_data(int parallel,FILE *fp,void *src,size_t size,char *myname,char *descrip)
{
  if (parallel != 0) 
    return pread_data(fp,src,size,myname,descrip);
  else 
    return sread_data(fp,src,size,myname,descrip);
}
/*---------------------------------------------------------------------------*/

int psread_byteorder(int byterevflag,int parallel,FILE *fp,void *src,size_t size,char *myname,char *descrip)
{
  if (parallel != 0) 
    return pread_byteorder(byterevflag,fp,src,size,myname,descrip);
  else 
    return sread_byteorder(byterevflag,fp,src,size,myname,descrip);
}
/*---------------------------------------------------------------------------*/
...
void pwrite_gauge_hdr(FILE *fp,gauge_header *gh)
{
  ...
  pwrite_data(fp,((void *)(&gh -> magic_number)),(sizeof(gh -> magic_number)),myname,"magic_number");
  pwrite_data(fp,((void *)(gh -> dims)),(sizeof(gh -> dims)),myname,"dimensions");
  pwrite_data(fp,((void *)(gh -> time_stamp)),(sizeof(gh -> time_stamp)),myname,"time_stamp");
  pwrite_data(fp,(&gh -> order),(sizeof(gh -> order)),myname,"order");
/* Header byte length */
  gh -> header_bytes = (sizeof(gh -> magic_number) + sizeof(gh -> dims) + sizeof(gh -> time_stamp) + sizeof(gh -> order));
/* pwrite_gauge_hdr */
}
/*----------------------------------------------------------------------*/
/* This subroutine writes the gauge configuration header structure */
...
void swrite_gauge_hdr(FILE *fp,gauge_header *gh)
{
  ...
  swrite_data(fp,((void *)(&gh -> magic_number)),(sizeof(gh -> magic_number)),myname,"magic_number");
  swrite_data(fp,((void *)(gh -> dims)),(sizeof(gh -> dims)),myname,"dimensions");
  swrite_data(fp,((void *)(gh -> time_stamp)),(sizeof(gh -> time_stamp)),myname,"time_stamp");
...
char *fmt,
/* address of starting data
					   floating point data must be
					   of type (float) */
char *src,
/* number of data items if > 1 */
int count,
/* byte stride of data if
                                           count > 1 */
int stride)
{
  int i;
...
/* Check for valid keyword */
  for (i = 0; (strlen(gauge_info_keyword[i]) > 0) && (strcmp(gauge_info_keyword[i],keyword) != 0); i++) ;
...
/* Write data */
  for (((k = 0) , (data = ((char *)src))); k < n; (k++ , (data += stride))) {
    fprintf(fpout," ");
    if (strstr(fmt,"s") != ((char *)((void *)0))) 
      fprintf(fpout,fmt,data);
    else if (strstr(fmt,"d") != ((char *)((void *)0))) 
      fprintf(fpout,fmt, *((int *)data));
    else if (strstr(fmt,"e") != ((char *)((void *)0))) 
      fprintf(fpout,fmt,((double )( *((float *)data))));
    else if (strstr(fmt,"f") != ((char *)((void *)0))) 
      fprintf(fpout,fmt,((double )( *((float *)data))));
    else if (strstr(fmt,"g") != ((char *)((void *)0))) 
      fprintf(fpout,fmt,((double )( *((float *)data))));
    else {
...
void write_gauge_info_file(gauge_file *gf)
{
  FILE *info_fp;
  gauge_header *gh;
  char info_filename[256UL];
  char sums[15 * sizeof(int ) - 4 * sizeof(void *) - sizeof(size_t )];
  gh = (gf -> header);
...
/* Open header file */
  if ((info_fp = fopen(info_filename,"w")) == ((FILE *)((void *)0))) {
...
/* Make sure compilation gave us a 32 bit integer type */
  1?((void )0) : __assert_fail("sizeof(int32type) == 4","io_lat4.c",496,"struct <unnamed> *setup_input_gauge_file(char *)");
  gh = ((gauge_header *)(malloc((sizeof(gauge_header )))));
  if (gh == ((gauge_header *)((void *)0))) {
...
/* Make sure compilation gave us a 32 bit integer type */
  1?((void )0) : __assert_fail("sizeof(int32type) == 4","io_lat4.c",536,"struct <unnamed> *setup_output_gauge_file()");
  gh = ((gauge_header *)(malloc((sizeof(gauge_header )))));
  if (gh == ((gauge_header *)((void *)0))) {
    printf("%s: Can\'t malloc gh\n",myname);
    terminate(1);
...
/* Remove trailing end-of-line character */
    if ((gh -> time_stamp)[strlen((gh -> time_stamp)) - 1] == 10) 
...
/* Only node 0 opens the requested file */
  if (this_node == 0) {
    ...
    if (fp == ((FILE *)((void *)0))) {
      printf("%s: Node %d can\'t open file %s, error %d\n",myname,this_node,filename, *__errno_location());
      fflush(stdout);
      terminate(1);
    }
...
/* Only node 0 knows about this file */
    gf -> fp = ((FILE *)((void *)0));
  gf -> filename = filename;
/* Not used for writing */
  gf -> byterevflag = 0;
/* Not used for writing */
  gf -> rank2rcv = ((int32type *)((void *)0));
  gf -> parallel = 0;
  return gf;
/* w_serial_i */
}
...
/* Read checksums with byte reversal */
  if (psread_byteorder((gf -> byterevflag),parallel,(gf -> fp),(&gf -> check.sum29),(sizeof(gf -> check.sum29)),myname,"checksum") != 0) 
    terminate(1);
  if (psread_byteorder((gf -> byterevflag),parallel,(gf -> fp),(&gf -> check.sum31),(sizeof(gf -> check.sum31)),myname,"checksum") != 0) 
    terminate(1);
  if ((gf -> check.sum29 != (test_gc -> sum29)) || (gf -> check.sum31 != (test_gc -> sum31))) 
    printf("%s: Checksum violation. Computed %x %x.  Read %x %x.\n",myname,(test_gc -> sum29),(test_gc -> sum31),gf -> check.sum29,gf -> check.sum31);
  else {
...
void write_checksum(int parallel,gauge_file *gf)
{
  ...
  pswrite_data(parallel,(gf -> fp),(&gf -> check.sum29),(sizeof(gf -> check.sum29)),myname,"checksum");
  pswrite_data(parallel,(gf -> fp),(&gf -> check.sum31),(sizeof(gf -> check.sum31)),myname,"checksum");
...
/* File stream pointer */
  off_t offset;
/* Size of coordinate list in bytes */
  off_t coord_list_size;
/* Size of header plus coordinate list */
  off_t head_size;
/* Location of checksum */
  off_t checksum_offset;
/* Size of checksum record */
  ...
  if (this_node == 0) {
    ...
    lbuf = ((su3_matrix *)(malloc(((4096 * 4) * sizeof(su3_matrix )))));
    if (lbuf == ((su3_matrix *)((void *)0))) {
...
/* No coordinate list was written because fields are to be written
    ...
    if (fseeko(fp,offset,0) < 0) {
      printf("w_serial: Node %d fseeko %lld failed error %d file %s\n",this_node,((long long )offset), *__errno_location(),(gf -> filename));
      fflush(stdout);
      terminate(1);
    }
  }
...
/* Pack tbufs in lbuf */
            memcpy(((void *)(lbuf + (4 * buf_length))),((void *)tbuf),(4 * sizeof(su3_matrix )));
/* Accumulate checksums - contribution from next site */
            for (((k = 0) , (val = ((u_int32type *)(lbuf + (4 * buf_length))))); k < 4 * ((int )(sizeof(su3_matrix ))) / ((int )(sizeof(int32type ))); (k++ , val++)) {
...
/* Position file pointer */
    if (fseeko(fp,checksum_offset,0) < 0) {
      printf("w_serial: Node %d fseeko %lld failed error %d file %s\n",this_node,((long long )offset), *__errno_location(),(gf -> filename));
      fflush(stdout);
      terminate(1);
    }
...
/* Read next 32 bits (without byte reversal) and hope we find it now */
    if (psread_data(parallel,fp,(&gh -> magic_number),(sizeof(gh -> magic_number)),myname,"magic number") != 0) 
      terminate(1);
  }
  else 
    sixtyfourbits = 0;
...
/* Lattice dimensions */
  for (j = 0; j < 4; j++) {
    if (psread_byteorder( *byterevflag,parallel,fp,((gh -> dims) + j),(sizeof((gh -> dims)[j])),myname,"dimensions") != 0) 
      terminate(1);
/* If 64 bit integers, then we have to read 4 more bytes get the
  ...
  if (((((gh -> dims)[0] != nx) || ((gh -> dims)[1] != ny)) || ((gh -> dims)[2] != nz)) || ((gh -> dims)[3] != nt)) {
...
/* Gauge field parameters */
  if (psread_byteorder( *byterevflag,parallel,fp,(&c1),(sizeof(float )),myname,"c1") != 0) 
    terminate(1);
  if (psread_byteorder( *byterevflag,parallel,fp,(&c2),(sizeof(float )),myname,"c2") != 0) 
    terminate(1);
...
/* Gauge field parameters */
  struct __unnamed_class___F0_L1081_C3_L72R__L73R__scope____SgSS2___variable_declaration__variable_type_int32typei__typedef_declaration_variable_name_L72R__L73R__scope____SgSS2____scope__n_descript__DELIMITER__L72R__L73R__scope____SgSS2___variable_declaration__variable_type__Ab_c_index_200_Ae__variable_name_L72R__L73R__scope____SgSS2____scope__descript__DELIMITER__L72R__L73R__scope____SgSS2___variable_declaration__variable_type_int32typei__typedef_declaration_variable_name_L72R__L73R__scope____SgSS2____scope__n_param__DELIMITER__L72R__L73R__scope____SgSS2___variable_declaration__variable_type__Ab_f_index_2_Ae__variable_name_L72R__L73R__scope____SgSS2____scope__param {
/* Number of bytes in character string */
  int32type n_descript;
/* Describes gauge field */
  char descript[200UL];
/* Number of gauge field parameters */
  int32type n_param;
/* GF parameters */
  float param[2UL];}gauge_field;
...
/* Read header, do byte reversal, 
     if necessary, and check consistency */
/* Lattice dimensions */
  if (psread_byteorder( *byterevflag,parallel,fp,(gh -> dims),(sizeof(gh -> dims)),myname,"dimensions") != 0) 
    terminate(1);
  if (((((gh -> dims)[0] != nx) || ((gh -> dims)[1] != ny)) || ((gh -> dims)[2] != nz)) || ((gh -> dims)[3] != nt)) {
/* So we can use this routine to discover the dimensions,
	 we provide that if nx = ny = nz = nt = -1 initially
...
/* Header byte length */
  if (psread_byteorder( *byterevflag,parallel,fp,(&gh -> header_bytes),(sizeof(gh -> header_bytes)),myname,"header size") != 0) 
    terminate(1);
/* Data order */
  if (psread_byteorder( *byterevflag,parallel,fp,(&gh -> order),(sizeof(gh -> order)),myname,"order") != 0) 
    terminate(1);
/* Length of gauge field descriptor */
  if (psread_byteorder( *byterevflag,parallel,fp,(&gauge_field.n_descript),(sizeof(gauge_field.n_descript)),myname,"n_descript") != 0) 
    terminate(1);
...
/* Read without byte reversal */
  if (psread_data(parallel,fp,gauge_field.descript,(sizeof(gauge_field.descript)),myname,"descrip") != 0) 
    terminate(1);
/* Assures termination of string */
  gauge_field.descript[gauge_field.n_descript - 1] = 0;
  printf("gauge_field.descript: %s\n",gauge_field.descript);
/* Number of gauge field parameters */
  if (psread_byteorder( *byterevflag,parallel,fp,(&gauge_field.n_param),(sizeof(gauge_field.n_param)),myname,"n_param") != 0) 
    terminate(1);
...
/* Gauge field parameters */
  for (j = 0; j < gauge_field.n_param; j++) {
    if (psread_byteorder( *byterevflag,parallel,fp,(gauge_field.param + j),(sizeof(gauge_field.param[j])),myname,"gauge param") != 0) 
      terminate(1);
...
/* Special processing for NERSC archive format files */
  if (ARCHYES == 1) {
    ( *(gf -> header)).order = 0;
    if (parallel != 0) {
      fprintf(stderr,"%s: Must use reload_serial with archive files for now.\n",myname);
      terminate(1);
    }
/* Reads the entire header of the archive file */
    hdr = qcdhdr_get_hdr(fp);
/* Get dimensions */
    if (qcdhdr_get_int("DIMENSION_1",hdr,(dims + 0)) == -1) 
      error_exit("DIMENSION_1 not present");
    if (qcdhdr_get_int("DIMENSION_2",hdr,(dims + 1)) == -1) 
      error_exit("DIMENSION_2 not present");
    if (qcdhdr_get_int("DIMENSION_3",hdr,(dims + 2)) == -1) 
      error_exit("DIMENSION_3 not present");
    if (qcdhdr_get_int("DIMENSION_4",hdr,(dims + 3)) == -1) 
      error_exit("DIMENSION_4 not present");
    for (i = 0; i < 4; i++) 
      (gh -> dims)[i] = dims[i];
/* Get archive checksum */
    if (qcdhdr_get_int32x("CHECKSUM",hdr,&chksum) == -1) 
...
/* Read date and time stamp */
    if (psread_data(parallel,fp,(gh -> time_stamp),(sizeof(gh -> time_stamp)),myname,"time stamp") != 0) 
      terminate(1);
/* Read header byte length */
    gh -> header_bytes = (sizeof(gh -> magic_number) + sizeof(gh -> dims) + sizeof(gh -> time_stamp) + sizeof(gh -> order));
/* Read data order */
    if (psread_byteorder(byterevflag,parallel,fp,(&gh -> order),(sizeof(gh -> order)),myname,"order parameter") != 0) 
      terminate(1);
  }
  return byterevflag;
/* read_gauge_hdr */
...
/* Node 0 alone opens the file and reads the header */
  if (this_node == 0) {
    ...
    if (fp == ((FILE *)((void *)0))) {
      printf("r_serial_i: Node %d can\'t open file %s, error %d\n",this_node,filename, *__errno_location());
      fflush(stdout);
      terminate(1);
    }
    gf -> fp = fp;
    byterevflag = read_gauge_hdr(gf,0);
  }
  else 
/* The other nodes don't know about this file */
    gf -> fp = ((FILE *)((void *)0));
...
/* File stream pointer */
  off_t offset;
/* Size of gauge configuration checksum record */
  off_t gauge_check_size;
/* Size of coordinate list in bytes */
  off_t coord_list_size;
/* Size of header plus coordinate list */
  off_t head_size;
/* Where we put the checksum */
  off_t checksum_offset;
  int rcv_rank;
  int rcv_coords;
  int destnode;
...
/* Position file for reading gauge configuration */
    offset = head_size;
    if (fseeko(fp,offset,0) < 0) {
      printf("%s: Node 0 fseeko %lld failed error %d file %s\n",myname,((long long )offset), *__errno_location(),filename);
      fflush(stdout);
      terminate(1);
    }
    buf_length = 0;
...
/* just copy links */
      if (destnode == 0) {
        i = node_index(x,y,z,t);
        memcpy(((void *)(lattice[i].link + 0)),((void *)(lbuf + (4 * where_in_buf))),(4 * sizeof(su3_matrix )));
      }
      else 
/* send to correct node */
{
        send_field(((char *)(lbuf + (4 * where_in_buf))),(4 * sizeof(su3_matrix )),destnode);
      }
      where_in_buf++;
    }
    else 
...
/* The receiving node does the byte reversal and then checksum,
    ...
    if (this_node == destnode) {
      if (byterevflag == 1) 
        byterevn(((int32type *)(lattice[i].link + 0)),(4 * sizeof(su3_matrix ) / sizeof(int32type )));
/* Accumulate checksums */
      for (((k = 0) , (val = ((u_int32type *)(lattice[i].link + 0)))); k < 4 * ((int )(sizeof(su3_matrix ))) / ((int )(sizeof(int32type ))); (k++ , val++)) {
...
/*   printf("lattice node_index = %d, mu = %d\n", i, mu); */
        memcpy(((void *)(lattice[i].link + 0)),((void *)(&tmpsu3)),(4 * sizeof(su3_matrix )));
/* send to correct node */
      }
      else {
...
/* All nodes open the same filename */
/* Returns a file structure describing the opened file */
  return parallel_open(0,filename);
/* w_parallel_i */
}
/*---------------------------------------------------------------------------*/
...
/* All nodes open the same filename */
/* Returns a file structure describing the opened file */
  return parallel_open(1,filename);
/* w_checkpoint_i */
}
/*---------------------------------------------------------------------------*/
...
/* File stream pointer */
  off_t offset;
/* Size of a gauge configuration block for
...
/* Each node writes its gauge configuration values */
  ...
  if (g_seek(fp,offset,0) < 0) {
    printf("%s: Node %d g_seek %ld failed error %d file %s\n",myname,this_node,((long )offset), *__errno_location(),(gf -> filename));
    fflush(stdout);
    terminate(1);
  }
  return lbuf;
...
/* gf  = file descriptor as opened by w_parallel_i */
  ...
  int z;
  int t;
  struct __unnamed_class___F0_L2091_C3_L74R__L75R__scope____SgSS2___variable_declaration__variable_type_s_variable_name_L74R__L75R__scope____SgSS2____scope__x__DELIMITER__L74R__L75R__scope____SgSS2___variable_declaration__variable_type_s_variable_name_L74R__L75R__scope____SgSS2____scope__y__DELIMITER__L74R__L75R__scope____SgSS2___variable_declaration__variable_type_s_variable_name_L74R__L75R__scope____SgSS2____scope__z__DELIMITER__L74R__L75R__scope____SgSS2___variable_declaration__variable_type_s_variable_name_L74R__L75R__scope____SgSS2____scope__t__DELIMITER__L74R__L75R__scope____SgSS2___variable_declaration__variable_type__Ab_su3_matrixL67R__typedef_declaration_index_4_Ae__variable_name_L74R__L75R__scope____SgSS2____scope__link {
  short x;
  short y;
  short z;
  short t;
  su3_matrix link[4UL];}msg;
  int isite;
  int ksite;
  int site_block;
  int rcv_coords;
...
/* Clear buffer as a precaution.  Easier to tell if we botch the
  ...
  for (i = 0; i < 4096; i++) 
    for (j = 0; j < 3; j++) 
      for (k = 0; k < 3; k++) {
        lbuf[i].e[j][k].real = (lbuf[i].e[j][k].imag = 0.0);
      }
...
/* MUST be a factor of MAX_BUF_LENGTH */
  ...
  for (ksite = 0; ksite < sites_on_node; ksite += site_block) {
    for (destnode = 0; destnode < number_of_nodes; destnode++) 
      for (isite = ksite; (isite < sites_on_node) && (isite < (ksite + site_block)); isite++) {
...
/* Message consists of site coordinates and 4 link matrices */
          ...
          msg.t = t;
          i = node_index(x,y,z,t);
          memcpy(((void *)msg.link),((void *)lattice[i].link),(4 * sizeof(su3_matrix )));
...
/* just copy links */
          if (destnode == sendnode) {
            ...
            memcpy(((void *)(lbuf + (4 * where_in_buf))),((void *)lattice[i].link),(4 * sizeof(su3_matrix )));
            rank29 = (rank31 = (4 * sizeof(su3_matrix ) / sizeof(int32type ) * rcv_rank));
          }
          else {
/* Receive a message */
...
/* Move data to buffer */
            memcpy(((void *)(lbuf + (4 * where_in_buf))),((void *)msg.link),(4 * sizeof(su3_matrix )));
...
/* Receiving node accumulates checksums as the values
		 are inserted into its buffer */
          rank29 %= 29;
          rank31 %= 31;
          for (((k = 0) , (val = ((u_int32type *)(lbuf + (4 * where_in_buf))))); k < 4 * ((int )(sizeof(su3_matrix ))) / ((int )(sizeof(int32type ))); (k++ , val++)) {
            gf -> check.sum29 ^= (( *val << rank29) | ( *val >> (32 - rank29)));
            gf -> check.sum31 ^= (( *val << rank31) | ( *val >> (32 - rank31)));
            rank29++;
            if (rank29 >= 29) 
...
/* To assure all write buffers are completed before
...
/* Only node 0 writes checksum data */
  if (this_node == 0) {
    if (g_seek(fp,checksum_offset,0) < 0) {
      printf("%s: Node %d g_seek %ld for checksum failed error %d file %s\n",myname,this_node,((long )checksum_offset), *__errno_location(),(gf -> filename));
      fflush(stdout);
      terminate(1);
    }
    write_checksum(1,gf);
...
/* load the gauge configuration into the buffer */
    memcpy(((void *)(lbuf + (4 * buf_length))),((void *)lattice[i].link),(4 * sizeof(su3_matrix )));
/* Accumulate checksums - contribution from next site moved into buffer*/
    ...
    buf_length++;
    if ((buf_length == 4096) || (i == (sites_on_node - 1))) {
/* write out buffer */
      fflush(stdout);
      if (((int )(g_write(lbuf,(4 * sizeof(su3_matrix )),buf_length,fp))) != buf_length) {
        printf("%s: Node %d gauge configuration write error %d file %s\n",myname,this_node, *__errno_location(),(gf -> filename));
...
/* All nodes set up a gauge file and guage header structure for reading */
  gf = setup_input_gauge_file(filename);
  gh = (gf -> header);
/* File was opened for parallel access */
  gf -> parallel = 1;
...
/* Node 0 reads header */
  if (this_node == 0) 
    byterevflag = read_gauge_hdr(gf,1);
/* Broadcast the byterevflag from node 0 to all nodes */
  broadcast_bytes(((char *)(&byterevflag)),(sizeof(byterevflag)));
  gf -> byterevflag = byterevflag;
...
/* gf  = gauge configuration file structure */
  ...
  int byterevflag;
  su3_matrix *lbuf;
  struct __unnamed_class___F0_L2452_C3_L76R__L77R__scope____SgSS2___variable_declaration__variable_type_s_variable_name_L76R__L77R__scope____SgSS2____scope__x__DELIMITER__L76R__L77R__scope____SgSS2___variable_declaration__variable_type_s_variable_name_L76R__L77R__scope____SgSS2____scope__y__DELIMITER__L76R__L77R__scope____SgSS2___variable_declaration__variable_type_s_variable_name_L76R__L77R__scope____SgSS2____scope__z__DELIMITER__L76R__L77R__scope____SgSS2___variable_declaration__variable_type_s_variable_name_L76R__L77R__scope____SgSS2____scope__t__DELIMITER__L76R__L77R__scope____SgSS2___variable_declaration__variable_type__Ab_su3_matrixL67R__typedef_declaration_index_4_Ae__variable_name_L76R__L77R__scope____SgSS2____scope__link {
  short x;
  short y;
  short z;
  short t;
...
/* Each node reads */
  offset += (gauge_node_size * this_node);
  if (g_seek(fp,offset,0) < 0) {
    printf("%s: Node %d g_seek %ld failed error %d file %s\n",myname,this_node,((long )offset), *__errno_location(),filename);
    fflush(stdout);
    terminate(1);
  }
/* initialize checksums */
...
/* Do byte reversal if needed */
          if ((gf -> byterevflag) == 1) 
            byterevn(((int32type *)(lbuf + (4 * where_in_buf))),(4 * sizeof(su3_matrix ) / sizeof(int32type )));
/* Accumulate checksums - contribution from next site */
          for (((k = 0) , (val = ((u_int32type *)(lbuf + (4 * where_in_buf))))); k < 4 * ((int )(sizeof(su3_matrix ))) / ((int )(sizeof(int32type ))); (k++ , val++)) {
            test_gc.sum29 ^= (( *val << rank29) | ( *val >> (32 - rank29)));
            test_gc.sum31 ^= (( *val << rank31) | ( *val >> (32 - rank31)));
...
/* just copy links */
          if (destnode == sendnode) {
            i = node_index(x,y,z,t);
            memcpy(((void *)lattice[i].link),((void *)(lbuf + (4 * where_in_buf))),(4 * sizeof(su3_matrix )));
          }
          else 
/* send to correct node */
{
/* Message consists of site coordinates and 4 link matrices */
            ...
            msg.z = z;
            msg.t = t;
            memcpy(((void *)msg.link),((void *)(lbuf + (4 * where_in_buf))),(4 * sizeof(su3_matrix )));
            send_field(((char *)(&msg)),(sizeof(msg)),destnode);
          }
          where_in_buf++;
        }
...
/* for all nodes other than node sendnode */
{
          if (this_node == destnode) {
            get_field(((char *)(&msg)),(sizeof(msg)),sendnode);
            ...
            memcpy(((void *)lattice[i].link),((void *)msg.link),(4 * sizeof(su3_matrix )));
          }
        }
/** end over the lattice sites in block on all nodes ***/
      }
/* To prevent incoming message pileups */
    g_sync();
/** end over blocks **/
  }
  free(lbuf);
...
/* Node 0 positions file for reading checksum */
    ...
    if ((gh -> magic_number) == 20103) {
      printf("Time stamp %s\n",(gh -> time_stamp));
      if (g_seek(fp,checksum_offset,0) < 0) {
        printf("%s: Node 0 g_seek %ld for checksum failed error %d file %s\n",myname,((long )offset), *__errno_location(),filename);
        fflush(stdout);
        terminate(1);
      }
      read_checksum(1,gf,&test_gc);
    }
    fflush(stdout);
  }
/* r_parallel */
...
/* Node 0 reads, and sends site to correct node */
          if (this_node == 0) {
            for (dir = 0; dir <= 3; dir++) {
              for (i = 0; i < 3; i++) 
                for (j = 0; j < 3; j++) {
                  if (fscanf(fp,"%e%e\n",&lbuf[dir].e[i][j].real,&lbuf[dir].e[i][j].imag) != 2) {
...
/* tell newnode it's OK to send */
          ...
          if (this_node == 0) {
            ...
            for (dir = 0; dir <= 3; dir++) {
              for (i = 0; i < 3; i++) 
                for (j = 0; j < 3; j++) {
                  if (fprintf(fp,"%.7e\t%.7e\n",((double )lbuf[dir].e[i][j].real),((double )lbuf[dir].e[i][j].imag)) == -1) {
...
gauge_file *restore_serial(char *filename)
{
  ...
  if (( *(gf -> header)).magic_number == 1111836489) {
    r_serial_arch(gf);
  }
  else {
    r_serial(gf);
          ...
          if (this_node == 0) {
            if (currentnode == 0) {
              ...
              for (mu = 0; mu < 4; ++mu) {
                for (a = 0; a < 2; ++a) {
                  for (b = 0; b < 3; ++b) {
                    uout[((2 * (b + (3 * a))) + (12 * mu)) + (48 * j)] = ((OUTPUT_TYPE )(s -> link)[mu].e[a][b].real);
                    uout[((1 + (2 * (b + (3 * a)))) + (12 * mu)) + (48 * j)] = ((OUTPUT_TYPE )(s -> link)[mu].e[a][b].imag);
                  }
                }
              }
            }
              ...
              for (mu = 0; mu < 4; ++mu) {
                for (a = 0; a < 2; ++a) {
                  for (b = 0; b < 3; ++b) {
                    uout[((2 * (b + (3 * a))) + (12 * mu)) + (48 * j)] = ((OUTPUT_TYPE )lbuf[mu].e[a][b].real);
                    uout[((1 + (2 * (b + (3 * a)))) + (12 * mu)) + (48 * j)] = ((OUTPUT_TYPE )lbuf[mu].e[a][b].imag);
                  }
                }
              }
            }
