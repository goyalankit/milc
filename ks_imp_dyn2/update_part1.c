/********** update.c ****************************************************/
/* MIMD version 6 */
/* first half of trajectory - checkpoint lattice and momenta at end */
...
float xrandom;
    ...
    /* Checkpoint lattice and momenta - filename hardcoded for now */
    {site *s; int i,dir; su3_matrix tmat; double h_action;
    h_action = hmom_action();
    save_lattice( SAVE_SERIAL, "/scratch/scratchdirs/toussain/checkfile.gauge" );
    FORALLSITES(i,s)for(dir=XUP;dir<=TUP;dir++)
	 uncompress_anti_hermitian( &(s->mom[dir]) , &(s->link[dir]) );
    save_lattice( SAVE_SERIAL, "/scratch/scratchdirs/toussain/checkfile.mom" );
    node0_printf("SAVED CHECKPOINT FIELDS, h_action = %e\n",h_action);
    g_sync(); exit(0); /* mucked up gauge field */
    }
