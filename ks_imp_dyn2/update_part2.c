...
float xrandom;
    ...
    /* DON'T refresh the momenta */
    /*ranmom();*/
    /* Instead, restore the momentum */
    /* Checkpoint lattice and momenta - filename hardcoded for now */
    {site *s; int i,dir; double h_action;
    reload_lattice( RELOAD_SERIAL, "/scratch/scratchdirs/toussain/checkfile.mom" );
    FORALLSITES(i,s)for(dir=XUP;dir<=TUP;dir++)
        make_anti_hermitian( &(s->link[dir]), &(s->mom[dir]) );
    reload_lattice( RELOAD_SERIAL, "/scratch/scratchdirs/toussain/checkfile.gauge" );
    h_action = hmom_action();
    node0_printf("RELOADED CHECKPOINT FIELDS, h_action = %e\n",h_action);
    }
