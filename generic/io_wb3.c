	  ...
	  if(currentnode==0)
	    {
	      ...
	      msg.wv = *((wilson_vector *)F_PT(&lattice[i],src));
	    }
	  else
	    {
  ...
  for(rcv_rank=0; rcv_rank<volume; rcv_rank++)
	...
	if(where_in_buf == buf_length)
	  {  /* get new buffer */
	    ...
	    if( (int)fread(lbuf,sizeof(wilson_vector),buf_length,fp) 
		!= buf_length)
	      {
		if(status == 0)
	...
	if(this_node==destnode){
	  i = node_index(x,y,z,t);
	  dest = (wilson_vector *)F_PT( &(lattice[i]), src );
	  /* Receive padded message in msg and copy to destination */
	  get_field((char *)&msg, sizeof(msg), 0);
	  *dest = msg.wv;
	}
      }
...
wilson_vector *w_parallel_setup_w(w_prop_file *wpf, off_t *checksum_offset,
			       int spin, int color)
{
  ...
  w_prop_node_size = sites_on_node*sizeof(wilson_vector) ;
  w_prop_size = volume*sizeof(wilson_vector) ;
  w_prop_check_size =  sizeof(wpf->check.spin) +
	...
	for(isite=ksite; 
	    isite<sites_on_node && isite<ksite+site_block; isite++)
	  {
	    ...
	    /* Node destnode receives a message */
	      ...
	      if(destnode==sendnode){ /* just copy Wilson vector */
		i = node_index(x,y,z,t);
		send = (wilson_vector *)F_PT( &(lattice[i]), src );
		where_in_buf = buf_length;
		lbuf[where_in_buf] = *send;

		rank29 = rank31 = 
		  sizeof(wilson_vector)/sizeof(int32type)*rcv_rank;
	      }
	      else {
		/* Receive a message */
		...
		rank29 = rank31 = sizeof(wilson_vector)/sizeof(int32type)*i;
	      }
		 ...
		 are inserted into its buffer */
		  ...
		  k < (int)sizeof(wilson_vector)/(int)sizeof(int32type); 
		  k++, val++)
		{
		  wpf->check.sum29 ^= (*val)<<rank29 | (*val)>>(32-rank29);
		  wpf->check.sum31 ^= (*val)<<rank31 | (*val)>>(32-rank31);
...
void w_checkpoint_w(w_prop_file *wpf, int spin, int color, field_offset src)
{
  ...
  FORALLSITES(i,s)
  {
	...
	if( (int)g_write(lbuf,sizeof(wilson_vector),buf_length,fp) != buf_length)
	  {
