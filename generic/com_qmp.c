...
initialize_machine(int argc, char **argv)
{
  ...
  i = QMP_init_msg_passing(argc, argv, QMP_SMP_MULTIPLE_ADDRESS);
  if(i!=0) {
    printf("%s\n", QMP_error_string(i));
...
{
  QMP_sum_float_array((float *)cpt, 2);
}
...
{
  QMP_sum_float_array((float *)cpt, 2*ncomplex);
}
...
{
  QMP_sum_float_array((float *)wvpt, 24);
}
...
{
  QMP_broadcast(fpt, sizeof(float));
}
...
{
  QMP_broadcast(cpt, 2*sizeof(float));
}
