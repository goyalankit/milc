...
int spectrum_multimom( float dyn_mass, float low_mass, float mass_inc, int nmasses, float tol){
  ...
  /* array of masses */
  j_dyn=-1;
  masses = (float *)malloc(nmasses*sizeof(float));
  for(i=0; i<nmasses; i++){
...
if( t_source%2 != 0 ){node0_printf("Even sources only!\n"); terminate(0);}
	    if( s->t==t_source && (s->x%2)==(s->t)%2 && (s->y%2)==0 && (s->z%2)==0 ){
		 ...
		 cc = ce_itheta(  2*PI*(s->x)/((float)nx) ) ; CSUM( s->quark_source.c[color], cc );
		 cc = ce_itheta(  2*PI*(s->y)/((float)ny) ) ; CSUM( s->quark_source.c[color], cc );
		 cc = ce_itheta(  2*PI*(s->z)/((float)nz) ) ; CSUM( s->quark_source.c[color], cc );
		/**
		s->quark_source.c[color].real += cos( 2*PI*(s->x)/((float)nx) );
		s->quark_source.c[color].real += cos( 2*PI*(s->y)/((float)ny) );
		s->quark_source.c[color].real += cos( 2*PI*(s->z)/((float)nz) );
		**/
	    }
	}
