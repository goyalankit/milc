...
int fpi_2( float *masses, int nmasses, float tol){
  ...
  for(src_count=0,t_source=source_start; t_source<nt && src_count<n_sources;
	...
	for(j=0;j<nmasses;j++){
	    dslash_on_temp( quark_props[j], temp_prop, EVENANDODD );
	    FORALLSITES(i,s){
  ...
  for(i=0;i<nmasses*nmasses;i++){
      for(j=0;j<nt;j++){
          CDIVREAL(props[NPROPS*i+POINTPOINT][j],(float)NSOURCEVECS*n_sources*nx*ny*nz*nx*ny*nz,
	    props[NPROPS*i+POINTPOINT][j]);
          CDIVREAL(props[NPROPS*i+POINTWALL ][j],(float)NSOURCEVECS*n_sources*nx*ny*nz*nx*ny*nz,
