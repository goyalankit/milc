...
/**printf("dslash_special: up gathers, start=%d\n",start);**/
	if(start==1){
	    tag[dir] = start_gather( src, sizeof(su3_vector),
	    dir, parity, gen_pt[dir] );
	}
	else {
            /**now we don't need to restart these gathers **/
	    /**restart_gather( src, sizeof(su3_vector),
    ...
    for(dir=X3UP; dir<=T3UP; dir++){
	    ...
	    dir, parity, gen_pt[dir] );
/* TRY THIS CODE */
/* set pointers for nearest neighbor transmitted sites by hand */
/* assumes third neighbor start_gather has set its gen_pt's */
FORSOMEPARITY(i,s,parity){
if( gen_pt[dir][i] < (char *)src || gen_pt[dir][i] >= (char *)(src+sites_on_node) ){
coords[XUP]=s->x; coords[YUP]=s->y; coords[ZUP]=s->z; coords[TUP]=s->t;
coords[dir]-=2;
/* optional partial check for whether the layout is OK for this trick */
/* really should fix this so it is done once, first time routine is called */
if(node_number(coords[XUP],coords[YUP],coords[ZUP],coords[TUP]) != this_node){terminate(0);}
gen_pt[dir][i]=gen_pt[DIR3(dir)][node_index(coords[XUP],coords[YUP],coords[ZUP], coords[TUP])];
}
} END_LOOP
	}
	else {
